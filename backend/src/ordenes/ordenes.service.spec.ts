...
describe('OrdenesService', () => {
  ...
  beforeEach(async () => {
    mockMesasService = { buscarPorId: jest.fn(), cambiarEstadoMesa: jest.fn() };
    mockProductosService = { buscarPorId: jest.fn() };
    mockRecetasService = { buscarPorProducto: jest.fn() };
    mockInventarioService = { buscarPorId: jest.fn(), descontarPorReceta: jest.fn() };
  ...
  describe('crearOrden', () => {
    it('crea ordenes separadas COCINA y CAFETERIA', async () => {
      mockMesasService.buscarPorId.mockResolvedValue({
        id: MESA_ID, numero: 5, capacidad: 4,
        estado: MesaEstado.OCUPADA,
        meseroActual: { id: MESERO_ID, nombre: 'Test' },
        abiertaEn: new Date(), cerradaEn: null,
      });

      mockProductosService.buscarPorId
        .mockResolvedValueOnce({
          id: PRODUCTO_COMIDA_ID, nombre: 'Hamburguesa', precio: 120,
          disponible: true, tipo: ProductoTipo.COMIDA,
          tiempoPreparacionMin: 15, calorias: 500, alergenos: ['gluten'],
        })
        .mockResolvedValueOnce({
          id: PRODUCTO_BEBIDA_ID, nombre: 'Coca Cola', precio: 25,
          disponible: true, tipo: ProductoTipo.BEBIDA,
          temperatura: 'FRIA', tamanosDisponibles: [],
        });
      ...
      mockInventarioService.buscarPorId.mockResolvedValue({
        id: INVENTARIO_ITEM_ID, nombre: 'Harina', unidad: 'KG',
        stockActual: 10, stockMinimo: 1, costoUnitario: 20, activo: true,
      });

      mockInventarioService.descontarPorReceta.mockResolvedValue(undefined);

      const docCocina = mockDoc({ _id: new Types.ObjectId(), tipo: TipoOrden.COCINA });
    ...
    it('lanza error si mesa no esta ocupada', async () => {
      mockMesasService.buscarPorId.mockResolvedValue({
        id: MESA_ID, estado: MesaEstado.LIBRE,
      });

      await expect(service.crearOrden(dtoBase, MESERO_ID))
    ...
    it('lanza error si producto no disponible', async () => {
      mockMesasService.buscarPorId.mockResolvedValue({
        id: MESA_ID, estado: MesaEstado.OCUPADA,
      });
      mockProductosService.buscarPorId
        .mockResolvedValueOnce({
          id: PRODUCTO_COMIDA_ID, nombre: 'Hamburguesa', precio: 120,
          disponible: false, tipo: ProductoTipo.COMIDA,
          tiempoPreparacionMin: 15, calorias: 500, alergenos: ['gluten'],
        });
    ...
    it('lanza error detallado si falta stock (valida antes de descontar)', async () => {
      ...
      mockInventarioService.buscarPorId.mockResolvedValue({
        id: INVENTARIO_ITEM_ID, nombre: 'Harina', unidad: 'KG',
        stockActual: 0.1, stockMinimo: 1, costoUnitario: 20, activo: true,
      });
  ...
  describe('marcarOrdenEntregada', () => {
    it('marca como entregada si todos los items estan LISTO', async () => {
      const doc = mockDoc({
        items: [
          { _id: new Types.ObjectId(ITEM_ID), estadoItem: ItemEstado.LISTO, producto: new Types.ObjectId() },
          { _id: new Types.ObjectId(), estadoItem: ItemEstado.LISTO, producto: new Types.ObjectId() },
        ],
        estadoGeneral: OrdenEstado.LISTA,
      });
      model.findById = jest.fn().mockReturnValue(mockQuery(doc));

      await service.marcarOrdenEntregada(doc._id.toHexString());
    ...
    it('lanza error si hay items pendientes', async () => {
      const doc = mockDoc({
        items: [
          { _id: new Types.ObjectId(), estadoItem: ItemEstado.PENDIENTE, producto: new Types.ObjectId() },
          { _id: new Types.ObjectId(), estadoItem: ItemEstado.LISTO, producto: new Types.ObjectId() },
        ],
      });
      model.findById = jest.fn().mockReturnValue(mockQuery(doc));

      await expect(service.marcarOrdenEntregada(doc._id.toHexString()))
        .rejects.toThrow(BadRequestException);
  ...
  describe('actualizarEstadoItem', () => {
    it('actualiza estado de un item especifico', async () => {
      const doc = mockDoc({
        items: [
          { _id: new Types.ObjectId(ITEM_ID), estadoItem: ItemEstado.PENDIENTE, producto: new Types.ObjectId() },
        ],
      });
      model.findById = jest.fn().mockReturnValue(mockQuery(doc));

      await service.actualizarEstadoItem(
        doc._id.toHexString(), ITEM_ID, ItemEstado.EN_PREPARACION,
      );
